...
<span id="syntax" class="md-typeset" style="display: block; font-weight: bold;">Syntax</span>
```
streamingml:kMeansIncremental(<STRING> model.name, <DOUBLE> decay.rate, <INT> no.of.clusters, <DOUBLE|FLOAT|INT|LONG> model.features)
```
...
<span id="syntax" class="md-typeset" style="display: block; font-weight: bold;">Syntax</span>
```
streamingml:kMeansMiniBatch(<STRING> model.name, <DOUBLE> decay.rate, <INT> no.of.clusters, <INT> max.iterations, <INT> no.of.events.to.retrain, <DOUBLE|FLOAT|INT|LONG> model.features)
```
